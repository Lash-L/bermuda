...
class BermudaDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    def __init__(
        ...
        @callback
        def handle_state_changes(ev: Event[EventStateChangedData]) -> None:
        ...
        @callback
        def handle_devreg_changes(ev: Event[EventDeviceRegistryUpdatedData]) -> None:
            """
    ...
    def sensor_created(self, address) -> None:
    ...
    def device_tracker_created(self, address) -> None:
    ...
    def count_active_scanners(self) -> int:
        ...
        for scanner in self.scanner_list:
            ...
            for device in self.devices.values():
                ...
                if record is not None and record.stamp is not None and record.stamp > last_stamp:
    ...
    async def _async_update_data(self) -> None:
        ...
        if self._do_full_scanner_init and not self._refresh_scanners():
        ...
        for address, device in self.devices.items():
            if device.create_sensor and (not device.create_sensor_done or not device.create_tracker_done):
            ...
            for pb_entry in pb_entries:
                ...
                for pb_entity in pb_entities:
                    if pb_entity.domain == DEVICE_TRACKER:
                        ...
                        pb_device = devreg.async_get(pb_entity.device_id) if pb_entity.device_id is not None else None
    ...
    def _refresh_area_by_min_distance(self, device: BermudaDevice) -> None:
        ...
        # FIXME: Don't use assert in code as it can be skipped when python is run with optimizations
    ...
    def _refresh_scanners(self, scanners: list[BluetoothScannerDevice] | None = None) -> bool:
        """
