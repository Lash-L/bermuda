...
class BermudaDeviceScanner(dict):
    ...
    def update_advertisement(
        ...
        if len(self.hist_stamp) == 0 or new_stamp is not None:
            ...
            _interval = new_stamp - self.stamp if new_stamp is not None and self.stamp is not None else None
    ...
    def calculate_data(self) -> None:
        ...
        if new_stamp is not None and self.rssi_distance is None:
            ...
            movavg = dist_total / dist_count if dist_count > 0 else local_min
    ...
    def to_dict(self):
        ...
        for var, val in vars(self).items():
            if var == "adverts":
                # FIXME: This is overriding the loop variable - assume not intended?
                val = {}  # noqa
