    ...
    async def async_step_init(self, user_input=None):  # pylint: disable=unused-argument
        ...
        if len(self.coordinator.scanner_list) == 0:
            messages["status"] = (
                ...
                "Any one of esphome bluetooth_proxy, Shelly bluetooth proxy or local bluetooth adaptor should "
                "get you started."
            )
    ...
    async def async_step_selectdevices(self, user_input=None):
        ...
        for device in self.devices.values():
            ...
            if device.address_type == ADDR_TYPE_IBEACON:
                ...
                source_mac = f"[{device.beacon_sources[0].upper()}]" if len(device.beacon_sources) > 0 else ""
